...
export default function CommissionerCCTVPage() {
    ...
    const [wardId, setWardId] = useState("demo_ward_1"); // Default ward for prototype
    ...
    useEffect(() => {
        ...
        loadData();
    }, [user, isSupervisor, isAdmin, router, loadData]);
        ...
        try {
            const res = await fetch(`http://localhost:8001/api/cctv/alerts/${selectedAlert.alert_id}/verify`, {
                ...
                body: JSON.stringify({ 
                    ...data, 
                    ward_id: wardId,
                    verifier_id: user?.id || 'commissioner_1',
                    verifier_name: user?.name || 'Commissioner',
                    verifier_role: 'commissioner'
                })
            });
            if (!res.ok) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-12">
            <div className="bg-gradient-to-r from-gray-900 to-indigo-900 text-white px-6 py-8">
                <div className="max-w-7xl mx-auto flex justify-between items-end">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                             <span className="bg-indigo-600 text-white text-xs font-bold px-2.5 py-1 rounded border border-indigo-500 shadow-sm flex items-center">
                               Commissioner Dashboard
                             </span>
                        </div>
                        <h1 className="text-3xl font-bold tracking-tight">City CCTV Overview</h1>
                        <p className="text-gray-400 mt-2 max-w-2xl text-sm">Monitor civic issues detected across city cameras. Currently viewing Ward {wardId}.</p>
                    </div>
                    <div className="flex gap-3">
                        <select 
                            value={wardId}
                            onChange={(e) => setWardId(e.target.value)}
                            className="bg-slate-800 border border-slate-700 text-white text-sm rounded-lg px-3 py-2 mr-2 focus:ring-indigo-500 focus:border-indigo-500"
                        >
                            <option value="demo_ward_1">Demo Ward 1</option>
                            <option value="demo_ward_2">Demo Ward 2</option>
                        </select>
                        <button 
                    ...
                    <div>
                        {filterCamera && (
                            <button 
                                ...
                                className="text-xs text-indigo-600 hover:underline mr-4"
                            >
